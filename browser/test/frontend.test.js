...
function harness() {
  ...
  const context = vm.createContext({
    ...
    setInterval:(fn, ms) => {intervals.push({fn,ms});}, setTimeout:() => 1, clearTimeout() {},
    location:{protocol:'http:',host:'test'}, AbortSignal,
    fetch:async url => ({ ok:true, json:async () => url === '/api/join' ? {token:'fixture'} : {seats:[]} }),
  });
  ...
  return {
    run, join, document, window, intervals,
