...
The bottom-left performance panel stays visible while aiming. It shows refresh-aligned presentation FPS, stream FPS, network RTT and p95 presentation gap. A single retained VideoFrame is replaced by newer decoder output until the next animation callback; superseded frames and frames held during teardown are closed. Hover the toolbar FPS metrics to see browser animation cadence; neither counter measures physical panel scanout. `window.tournamentDiagnostics()` provides the same counters plus replaced frames and gaps over 50 ms without exposing tokens. Mouse forwarding preserves a 120 Hz deadline through small callback jitter, with no catch-up burst after idle time.
...
The browser's Resolution selector exposes 720p, 1080p and 1440p. It persists `tournament-resolution` locally and reapplies it after joining. No automatic resolution adaptation exists. The FPS counter reports refresh-aligned presentations independently of the requested size.
