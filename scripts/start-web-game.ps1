...
while($true) {
    ...
    try {
        ...
        foreach($seat in 0,1) {
            ...
            $client=& "$PSScriptRoot\join-game.ps1" -SourceRoot $SourceRoot -Name $name -StreamSeat $seat -HardwareVideo:$HardwareVideo -StreamFPS $StreamFPS -StreamResolution $StreamResolution
