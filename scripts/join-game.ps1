...
if ($StreamSeat -ge 0) {
    ...
    $streamSize=if(!$HardwareVideo){@(960,540)}elseif($StreamResolution -eq '1080p'){@(1920,1080)}elseif($StreamResolution -eq '720p'){@(1280,720)}else{@(960,540)}
    $arguments+=@(("-TournamentFramePort="+(9001+$StreamSeat)),("-TournamentInputPort="+(9101+$StreamSeat)),("-TournamentServer="+$Server),
        '-LogCmds="LogD3D11RHI Log"',('-ResX='+$streamSize[0]),('-ResY='+$streamSize[1]),'-ForceRes','-TournamentOffscreen','-nosound','-unattended',('-TournamentStreamFPS='+$StreamFPS),('-ExecCmds="t.MaxFPS '+$renderFPS+',r.VSync 0,t.IdleWhenNotForeground 0,r.OneFrameThreadLag 0,r.ScreenPercentage 100,r.MotionBlurQuality 0,r.DepthOfFieldQuality 0,r.DefaultFeature.AntiAliasing 1,r.MaxAnisotropy 16"'))
