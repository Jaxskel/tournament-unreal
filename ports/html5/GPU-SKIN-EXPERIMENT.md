...
The legacy Emscripten macro requires a dummy argument: the browser capability query uses `EM_ASM_INT({ ... }, 0)`. The patcher can migrate the exact earlier empty-varargs runtime body (normalized body SHA-256 `340da2ed7f854ebc3f0d50625f5da0535bb2acc8e0b816219fd1ca4a8306d0fa`) only with its verified original backup. It checks the entire previous file against the transformation of that backup, preserves the backup, and refuses unknown modifications or missing backups. The shader-cache predicate and JavaScript query are unchanged. This correction affects only the browser branch; an already built native editor does not need rebuilding for this dummy argument alone. Existing fresh shader-cook and coherent-package requirements still apply.
