...
**The local browser game is not ready.** The isolated engine now renders Deck, hands and the first-person weapon locally at fixed 1920×1080. A later missing shader uniform buffer and a separate multiplayer bounds assertion still block stable gameplay. Single-client reconnect passes with direct package loading. Commands below form a reproduction recipe; the complete sequence has not yet been replayed from a clean checkout.
...
| --- | --- |
| Matching legacy build | The latest Outline/AsyncIO wrapper build passed in **349.83 seconds** with all nine exports and no unresolved symbols. It includes configuration, logging, Party, pacing and browser window fixes. A fresh clean-checkout replay remains required. |
...
| Actual launcher probe | Chrome runs the actual WASM world and six-bot match locally, with native window/canvas/backing buffer at 1920×1080. Removed Web Audio velocity calls and cubemap mip activation have been corrected. The redundant startup I/O cleanup burst is fixed in the new build. **The Outline fix restores visible 3D rendering, but a later OpenGLShaders.cpp null uniform-buffer assertion stops sustained play. No valid gameplay FPS result.** |
...
| Final rotation / networking | Two independent Chrome contexts joined the isolated native 7797 server with distinct identities and simultaneous bidirectional traffic. Direct package loading fixed the observed Blob-storage failure and passed a real single-client reconnect. A stable two-client run reached readiness, then hit a native Array.h bounds assertion before Reconnect. V3 contains the rotation maps; sustained multiplayer, travel and performance remain unverified. |
