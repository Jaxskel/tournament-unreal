...
**The local browser game is not ready.** The isolated engine now renders Deck, hands and the first-person weapon locally at fixed 1920×1080. The HUD tile uniform-buffer fix passed its initial runtime check. Confirmed fall-through at a BSP floor and a separate late-join bounds assertion still block stable gameplay. Single-client reconnect passes with direct package loading. Commands below form a reproduction recipe; the complete sequence has not yet been replayed from a clean checkout.
...
| --- | --- |
| Matching legacy build | The latest Canvas tile-light wrapper build passed in **340.15 seconds** with all nine exports and no unresolved symbols. It includes configuration, logging, Party, pacing and browser window fixes. A fresh clean-checkout replay remains required. |
...
| Final rotation / networking | Two independent Chrome contexts joined the isolated native 7797 server with distinct identities and simultaneous bidirectional traffic. Direct package loading fixed the observed Blob-storage failure and passed a real single-client reconnect. A stable two-client run reached readiness, then hit a native Array.h bounds assertion before Reconnect. The assertion is mapped to intro-timer rescheduling against the wrong array after a late join. The bounded patch passes 17 actual-source checks; its Windows build and multiplayer rerun remain pending. V3 contains the rotation maps; sustained multiplayer, travel and performance remain unverified. |
