...
## Pinned one-master tessellation correction and shader diagnostics

`WeaponTessellationUpgrade.h` provides a separate, narrowly pinned seven-to-eight-node operation. Material instances can request the unused tessellation multiplier through the attributes graph even when the base material disables tessellation. The correction overrides only its ES2 value with the original constant one. It retains WPO, passthrough UV0, the native/default graph and material tessellation mode.

Use exactly one operation flag: `-WeaponTessUpgrade`, `-WeaponTessVerify`, or `-WeaponShaderProbe`. These cannot be combined with each other or any `-Mode=` operation. Shader selectors also require `-WeaponShaderProbe`. Conflicts fail before any helper dispatch; reusing an Apply command line for a diagnostic is rejected.

The upgrade requires `-WeaponUV0 -WeaponTess1` and the independently reviewed current-generation proof, original baseline/recipe, existing semantic-verification receipt, a separate one-master save receipt, verified external beforeimage and exclusive aftermath path. Argument names are `WeaponCurrentProof=`, `WeaponRepairSpec=`, `WeaponBaseline=`, `Receipt=`, `WeaponTessReceipt=`, `WeaponMasterBackup=` and `WeaponTessAftermath=`. Fresh verification also requires the exact `WeaponTessAftermathSHA1=`. These are private, generation-specific inputs; the tool does not accept arbitrary current assets or regenerate its own authority. Only the cloned master may be saved. The other 22 tracked packages, original dependencies and material-instance properties remain checked. The original Apply/Verify variants are unchanged.

The shader probe first performs full fresh verification, then owns six ES2 material resources: Enforcer first/third person at each quality. It reports map validity, errors, sampler usage and observed translation, with no asset saves. `-WeaponShaderEnforcer1PHigh` selects one diagnostic resource and emits a distinct completion marker. `-WeaponShaderNoStaticLighting` also selects that resource and overrides only its resource-level static-lighting getter and persistence. It does not change a material flag or CVar. Nonpersistence also changes compiler scheduling and component-recreation policy. Its completion is explicitly counterfactual, never ordinary acceptance.

The native counterfactual passed at 16 samplers. All 123 retained generated shaders and preprocessed sources matched the failing ordinary run byte-for-byte. It excluded 25 LocalVF lightmap variants, including 12 over-limit variants; both skeletal families retained 41 variants each. This establishes the compiler mechanism for one material/quality, not full material coverage, browser rendering or permission to discard lighting used by actual components. Complete usage evidence and all relevant material permutations remain prerequisites for a permanent repair.
...
python3 -B ports/html5/compat/test_commandlet_mode_admission.py -v
python3 -B ports/html5/compat/test_weapon_tessellation.py --private-source-dir /private/weapon-fidelity-source --private-log /private/fidelity-native-1/report.log --private-proof /private/uv0-current23-proof.json -v
python3 -B ports/html5/compat/test_weapon_shader_probe.py --private-source-dir /private/weapon-fidelity-source --private-log /private/fidelity-native-1/report.log -v
```
