...
Each launch defines `Module.canvas`, `arguments`, `preInit`, `preRun`, `postRun`, `onRuntimeInitialized`, `onAbort`, `onExit`, `monitorRunDependencies`, `setStatus`, `noImageDecoding: true` and `noAudioDecoding: true` before loading scripts. Assets are fetched once from the server, then exposed as frame-owned Blob URLs through `locateFile`. Legacy packagers can XHR these local Blob URLs without another network download. Undeclared asset requests fail explicitly. This supports the old file packager without relying on newer `getPreloadedPackage` APIs. Whole-asset blobs, WASM compilation and heap allocation can require substantial peak browser memory. Assets/Blob URLs remain owned by the disposable runtime frame until Stop/Retry.
