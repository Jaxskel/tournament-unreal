...
| `TournamentBrowserSessionEpoch` | none | double, positive controller/world generation when ready; `0` unavailable. Native implementation uses weak object identity to detect replacement even with address reuse. |
...
| `TournamentBrowserSetMenuPaused` | int 0/1, double expected epoch | Optional; `1` confirms the requested standalone menu pause/resume, `0` rejects or remains pending. |
| `TournamentBrowserMenuPauseStatus` | double expected epoch | Optional; `-1` unavailable/stale/network world, `0` running, `1` menu-owned effective pause, `2` other pause, `3` menu-owned pause waiting to take effect. |

Advertise the two menu exports together only after rebuilding the native plugin and verifying they exist. Older manifests with the original nine exports still launch, apply settings and resume input; their menus report that native practice pause is unavailable. The runtime verifier continues requiring only those original nine exports.

Practice menu pause uses `APlayerController::SetPause`, not suspension of the JavaScript main loop. Native calls require a ready local controller, PlayerState, auth game mode, exact current positive epoch, standalone net mode, no game net driver and no pending connection. The control remembers only its weak controller/world ownership, never adopts an existing foreign pause, avoids duplicate pause delegates, and confirms effective world state instead of trusting `ClearPause`'s return. Controller destruction uses the engine's existing pause cleanup. Multiplayer never invokes either menu pause export and keeps its networking loop running.

Menu intent survives loading and epoch changes. Resume/Capture releases inputs and waits for a native frame that both began and completed while the menu-owned world pause was effective; `GFrameCounter` must advance. This lets the engine consume a background/suspension time gap while gameplay is paused. Reopening Menu cancels pending resume. Movement/fire remain blocked until resume is confirmed; capture and audio activation still execute synchronously in the capture gesture. Native ticks continue to process deferred resolution and volume changes while paused, and saved volume is never replaced with a mute value. World pause uses UE's audio pause processing; actual HTML5 audio behavior still requires engine verification.
