...
| Current rotation retry | A current-generation headless retry reached Deck Ready but stopped on a legacy account-stats HTTP 404 before completing travel. A [browser NoMCP writer guard](CLOUD-STATS.md) is implemented and passes local source/fixture tests; its native build and browser verification remain pending. It is not in the playable build. The failed run also recorded cleanup timeouts, followed by owned-process exit. |
...
| Weapons and other assets | Wrong/gray weapon materials remain visible in the playable generation. Native graph repairs and isolated shader comparisons are underway; a saved, freshly cooked and visually checked repair is not yet established. Foot-shadow, lighting, effects, animation and weapon-placement fidelity still require acceptance. |
