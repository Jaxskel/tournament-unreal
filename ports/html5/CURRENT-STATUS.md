...
| Bots, combat and respawn | An older controlled practice capture ties the player controller to its player state, records Enforcer ammunition dropping from 40 to 27 after console-triggered firing, observes one death and no possessed pawn, then a new possessed pawn at 100 health after a respawn request. The pawn was already dead before the diagnostic Suicide command, so that command is not attributed as the cause. This is an older, console-controlled observation, not normal mouse combat, a player-shot kill or certification of the current generation. |
