"""Suppress global editor notification windows only for the opt-in Entry diagnostic.

Default mode is read-only. --apply makes an exact original backup and replaces
the one pinned GlobalEditorNotification.cpp file. No build, browser run, or asset
...
BACKUP = '.before-tournament-entry-reflection-no-notifications'
SPEC = {'path': 'Engine/Source/Editor/UnrealEd/Private/GlobalEditorNotification.cpp',
 'source_sha256': 'a540dabc6e921b0f4463a275827b2688238d30b465d05bf422ef474058c36ed0',
 'source_bytes': 2757,
 'output_sha256': 'a53bce9be1b8f5fa4d98e75c889a8dea201b4faa0d410130aefe259a335d5a2e',
 'output_bytes': 3706,
 'old': b'TSharedPtr<SNotificationItem> FGlobalEditorNotification::BeginNotification()\n{',
 'new': b'#include "Misc/App.h"\n#include "Misc/CommandLine.h"\n#include "Misc/Parse.h"\n\nTSharedPtr<SNotificatio'
        b'nItem> FGlobalEditorNotification::BeginNotification()\n{\n#if PLATFORM_WINDOWS\n\t// This diagnostic sup'
        b'presses presentation; notification work polling remains active.\n\tconst bool bTournamentEntryReflecti'
        b'onNoMainFrame = FApp::IsUnattended()\n\t\t&& FParse::Param(FCommandLine::Get(), TEXT("TournamentEntryRe'
        b'flectionNoMainFrame"))\n\t\t&& FParse::Param(FCommandLine::Get(), TEXT("EntryReflectionDiagnostic")'
        b')\n\t\t&& !FParse::Param(FCommandLine::Get(), TEXT("immersive"))\n\t\t&& !FParse::Param(FCommandLine::'
        b'Get(), TEXT("VREditor"))\n\t\t&& !FParse::Param(FCommandLine::Get(), TEXT("ForceVREditor"))\n\t\t&& !F'
        b'Parse::Param(FCommandLine::Get(), TEXT("AutomatedMapBuild"))\n\t\t&& !FParse::Param(FCommandLine::Get()'
        b', TEXT("nullrhi"))\n\t\t&& !FParse::Param(FCommandLine::Get(), TEXT("game"));\n#else\n\tconst bool bTo'
        b'urnamentEntryReflectionNoMainFrame = false;\n#endif\n\tif (bTournamentEntryReflectionNoMainFrame)\n\t'
...
def inverse(data, spec=SPEC):
    ...
    if data.count(spec['new']) != 1:
        raise ValueError('Expected one exact diagnostic notification guard: ' + spec['path'])
...
def transform(data, spec=SPEC):
    ...
    if data.count(spec['old']) != 1 or spec['new'] in data:
        raise ValueError('Expected exactly one unmodified notification anchor')
...
def _publish_backup(saved, destination, check_all):
    temporary = None
    try:
        with tempfile.NamedTemporaryFile(dir=destination.parent, prefix='.entry-notification-backup-', delete=False) as stream:
...
def patch(root, apply=False, spec=SPEC):
    ...
    if apply:
        ...
        if current['data'] != updated:
            temporary = None
            try:
                with tempfile.NamedTemporaryFile(dir=current['path'].parent, prefix='.entry-notification-patch-', delete=False) as stream:
                    temporary = Path(stream.name)
                ...
                if temporary is not None and temporary.exists():
                    temporary.unlink()
        check_all()
    return {'apply': apply, 'entryReflectionNotificationDiagnosticOnly': True, 'requiresFreshUnrealEdBuild': True,
